...
from protein_ebm.data.protein_utils import atom14_to_atom37
import sys
import traceback

class ProteinNoisedDataset(torch.utils.data.Dataset):
    def __init__(self, atom37, atom37_mask, residue_idx, aatype, residue_mask, contacts, diffuser, max_t=1.0, chain_ids=None, diffuse_sidechain=False, sidechain_drop_prob=0.0, max_len=1000, pad_to_max_threshold=None, fixed_noise_time=None, noise_time_mean=None, noise_time_std=None, noise_time_min=0.01):
        """
        Args:
            ...
            atoms_per_window_queries (int, optional): If provided, pad flattened atom features to be a multiple of this value (default: None)
            fixed_noise_time (float, optional): If provided, use this fixed time value for all samples instead of randomly sampling (default: None)
            noise_time_mean (float, optional): If set with noise_time_std, sample time from Normal(mean, std) then clamp to [noise_time_min, max_t]
            noise_time_std (float, optional): Standard deviation for normal time sampling (used when noise_time_mean is set)
            noise_time_min (float, optional): Lower bound for time clamping (default: 0.01)
        """
        ...
        self.target_ids = target_ids  # Store target ids
        ...
        self.fixed_noise_time = fixed_noise_time  # Fixed noise time if specified
        self.noise_time_mean = noise_time_mean
        self.noise_time_std = noise_time_std
        self.noise_time_min = noise_time_min

    def __len__(self):
        return self.num_proteins
    
    def __getitem__(self, idx):
        # Sample noise level for this example
        if self.fixed_noise_time is not None:
            t = torch.tensor([self.fixed_noise_time])
        elif self.noise_time_mean is not None and self.noise_time_std is not None:
            t = torch.normal(mean=torch.tensor([self.noise_time_mean]), std=torch.tensor([self.noise_time_std]))
            t = torch.clamp(t, self.noise_time_min, self.max_t)
        else:
            t = torch.clamp(torch.rand(1) * self.max_t, 0.01, None)  
        
        # retrieve relevant data
        atom37_mask = self.atom37_mask[idx]
        atom37_positions = self.atom37[idx]
        residue_mask = self.residue_mask[idx]
        residue_idx = self.residue_idx[idx]
        aatype = self.aatype[idx]
        contacts = self.contacts[idx]
        chain_ids = self.chain_ids[idx]

        # Convert atom14 to atom37 if dataloader received atom14 in atom37 fields
        if atom37_positions.shape[-2] == 14:
            atom37_positions, atom37_mask = atom14_to_atom37(atom37_positions, atom37_mask, aatype)

        # Determine crop indices
        seq_len = residue_mask.shape[0]
        ...
        if self.diffuse_sidechain:
            coords = atom37_positions  # [N, 37, 3]
            if torch.rand(1) < self.sidechain_drop_prob:
                atom37_mask = atom37_mask.clone()
                atom37_mask[..., 3:] = 0
                coords = coords.clone()
                coords[..., 3:, :] = 0
        else:
            coords = atom37_positions[:, 1, :]  # [N, 3]
        ...
        if self.diffuse_sidechain:
            mask = torch.zeros_like(atom37_mask)
            mask[start_idx:end_idx] = atom37_mask
            mask = mask.reshape(1, -1)
        else:
            mask = torch.zeros(1, atom37_positions.shape[0])
            mask[0, start_idx:end_idx] = 1.0
        
        # Apply random augmentation to coordinates
        aug_coords, atom37_aug = center_random_augmentation(
            flat_coords, mask, 
            second_coords=atom37_positions.reshape([1,-1,3]), 
        ...
        if residue_idx.dtype != torch.long:
            residue_idx = residue_idx.to(torch.long)
        ...
        output = {
            ...
            't': t,
            'chain_encoding': chain_ids,
            'id' : self.target_ids[idx] if self.target_ids is not None else 'None'
        }
        ...
        if start_idx != 0 or end_idx != seq_len:
            ...
            for key in output:
                if output[key] is None or key == 'id':
                    continue

                if len(output[key].shape) > 0:
                    if output[key].shape[0] == seq_len:
                        # Residue-level keys need cropping
