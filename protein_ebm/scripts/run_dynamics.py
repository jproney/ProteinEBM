...
parser.add_argument('--resample_dynamics_config', type=str, default="", help='Path to config for resample dynamics model (default: use main config)')
parser.add_argument('--lo_time_config', type=str, default="", help='Path to config for low time model')
parser.add_argument('--lo_time_checkpoint', type=str, default="", help='Path to checkpoint for low time model')
parser.add_argument('--lo_time_threshold', type=float, default=-1, help='Time threshold below which to use low time model (default: -1, no switching)')
...
parser.add_argument('--rescore_only', action='store_true', help='Only load previous samples and rescore with scoring checkpoint (no dynamics); requires --load_previous and --scoring_time')
...
parser.add_argument('--temp_scaling_after_lo_time', type=float, default=-1, help='Temperature scaling factor when t >= lo_time_threshold (default: -1, same as current temp_scaling)')
...
parser.add_argument('--residue_range', type=str, default=None, help='Residue range to use from PDB (format: "start-end" or "start:end", e.g., "10-50")')
parser.add_argument('--crop_external_contacts', action='store_true', help='When using --residue_range, mark residues with contacts to excluded parts of the chain')
parser.add_argument('--unknown_contacts', action='store_true', help='Set external_contacts to 0 (unknown) for all residues')
...
if args.rescore_only:
    if not args.load_previous:
        parser.error("--rescore_only requires --load_previous")
    if args.scoring_time == -1:
        parser.error("--rescore_only requires --scoring_time (e.g. --scoring_time 0.001)")
...
# Load low time model if specified
lo_time_model = None
if args.lo_time_checkpoint and args.lo_time_threshold != -1:
    print(f"Loading low time model for t < {args.lo_time_threshold}")
    lo_time_config_path = args.lo_time_config if args.lo_time_config else args.config
    with open(lo_time_config_path, 'r') as f:
        lo_time_config = yaml.safe_load(f)
    lo_time_config = ConfigDict(lo_time_config)
    lo_time_model = ProteinEBM(lo_time_config.model, diffuser).cuda()
    lo_time_ckpt = torch.load(args.lo_time_checkpoint, weights_only=False)
    if "diffuse_sidechain" not in lo_time_config.model:
        lo_time_config.model.diffuse_sidechain = False
    lo_time_model.load_state_dict({k[len("model."):]: v for k, v in lo_time_ckpt['state_dict'].items() if k.startswith('model')})
    print(f"Low time model loaded from {args.lo_time_checkpoint}")
...
if args.scoring_checkpoint:
    ...
    scoring_model = ProteinEBM(scoring_config.model, diffuser).cuda()
...
elif lo_time_model is not None:
    # Use low time model for scoring if no explicit scoring model specified
    scoring_model = lo_time_model
    print("Using low time model for scoring")
else:
    scoring_model = model
...
resample_dynamics_config_path = args.resample_dynamics_config if args.resample_dynamics_config else None
final_checkpoint_loaded = False
resample_dynamics_checkpoint_loaded = False
resample_dynamics_model = None  # separate model instance when resample config differs
...
def load_resample_dynamics_checkpoint():
    """Load resample dynamics checkpoint. Creates a new model if a separate config is given."""
    global resample_dynamics_checkpoint_loaded, resample_dynamics_model
    if resample_dynamics_checkpoint_path and not resample_dynamics_checkpoint_loaded:
        ...
        if resample_dynamics_config_path:
            print(f"Using separate config for resample dynamics model: {resample_dynamics_config_path}")
            with open(resample_dynamics_config_path, 'r') as f:
                resample_dynamics_cfg = ConfigDict(yaml.safe_load(f))
            resample_dynamics_model = ProteinEBM(resample_dynamics_cfg.model, diffuser).cuda()
            resample_dynamics_model.load_state_dict({k[len("model."):]: v for k, v in resample_dynamics_ckpt['state_dict'].items() if k.startswith('model')})
            resample_dynamics_model.eval()
            print("Resample dynamics model loaded with separate config")
        else:
...
def get_dynamics_model(t):
    """Get the appropriate model to use based on time threshold and round."""
...
def find_close_residues(target_residue, neighbor_search, distance_cutoff=8.0):
    """Find residues within distance_cutoff of target_residue using CB (or CA if no CB)."""
    # Get CB or CA atom from target residue
    target_atom = target_residue['CB'] if 'CB' in target_residue else target_residue['CA']
    
    # Find all atoms within the distance cutoff
    close_atoms = neighbor_search.search(target_atom.coord, distance_cutoff)
    
    # Collect unique residues from the close atoms
    close_residues = set()
...
def compute_crop_external_contacts(cropped_residues, all_residues, linear_thresh=6):
    """
    Compute external contacts for cropped residues based on contacts with excluded residues.
    
    Args:
        cropped_residues: List of residues in the cropped region
        all_residues: List of all residues in the full chain
        linear_thresh: Minimum sequence distance to consider as external contact
    
    Returns:
        List of contact flags (2 if has contact with excluded region, 1 otherwise)
    """
    # Create a set for fast lookup
    cropped_set = set(cropped_residues)
    
    # Get CA/CB atoms from all residues for neighbor search
    ca_cb = [
        (residue['CB'] if 'CB' in residue else residue['CA']) 
        for residue in all_residues if ('CA' in residue or 'CB' in residue)
    ]
    neighbor_search = NeighborSearch(ca_cb)
    
    # For each cropped residue, check if it has contacts with excluded residues
    contact_flags = []
    for r in cropped_residues:
        # Find close residues
        close_residues = find_close_residues(r, neighbor_search)
        
        # Filter to only include residues NOT in the cropped set
        # and with sufficient sequence distance
        inter_contacts = [
            r2 for r2 in close_residues 
            if r2 not in cropped_set and abs(int(r2.id[1]) - int(r.id[1])) > linear_thresh
        ]
        
        # Set flag to 2 if has contacts with excluded region, otherwise 1
        contact_flags.append(2 if len(inter_contacts) > 0 else 1)
...
all_residues = [r for r in chain.get_residues() if is_aa(r)]

# Filter residues by range if specified
full_chain_residues = None
if args.residue_range:
    # Store full chain for external contacts computation if needed
    if args.crop_external_contacts:
        full_chain_residues = all_residues
    
    # Parse residue range (supports "start-end" or "start:end" format)
    delimiter = '-' if '-' in args.residue_range else ':'
    start_res, end_res = map(int, args.residue_range.split(delimiter))
    
    # Filter residues by their ID
    filtered_residues = [r for r in all_residues if start_res <= r.id[1] <= end_res]
    
    print(f"Using residue range {start_res}-{end_res}: {len(filtered_residues)}/{len(all_residues)} residues")
    all_residues = filtered_residues
    
    if len(all_residues) == 0:
        raise ValueError(f"No residues found in range {start_res}-{end_res}")

atom_positions, atom_mask, aatype, residue_idx = residues_to_features(all_residues)
residue_idx = torch.tensor([r.id[1] for r in all_residues])
...
# Compute external contacts
if args.unknown_contacts:
    # Set all contacts to 0 (unknown)
    external_contacts = torch.zeros(aatype.shape, dtype=torch.long)
elif args.crop_external_contacts and full_chain_residues is not None:
    # Compute contacts with excluded parts of the chain
    print("Computing external contacts for cropped chain...")
    contact_flags = compute_crop_external_contacts(all_residues, full_chain_residues)
    external_contacts = torch.tensor(contact_flags, dtype=torch.long)
    print(f"Found {(external_contacts == 2).sum()} residues with external contacts")
else:
    # By default, don't include external_contacts in the feature dictionary
...
if args.load_previous:
    ...
    # Rescore-only mode: load scoring checkpoint, rescore, save and exit
    if args.rescore_only:
        print("Rescore-only mode: loading scoring checkpoint and rescoring all structures...")
        ...
        for i in range(0, len(final_structures)):
            ...
            if external_contacts is not None:
                rescore_input_feats['external_contacts'] = external_contacts.unsqueeze(0).expand([rescoring_batch_size, -1]).cuda()
            with torch.no_grad():
                rescore_out = scoring_model.compute_energy(rescore_input_feats)
                rescored_energies.append(rescore_out['energy'].detach().cpu())
            if (i + 1) % 10 == 0 or i == len(final_structures) - 1:
                print(f"  Rescored batch {i + 1}/{len(final_structures)}")
        # One energy per structure (min over residues, matching rescore_previous behavior)
        rescored_min = torch.stack([e.min() for e in rescored_energies])
        n_frames = prev_data['all_t0'][0].shape[1]
        all_scoring_energies = [
            [rescored_min[b * n_frames : (b + 1) * n_frames]]
            for b in range(nbatch)
        ]
        save_dict = {
            'all_pos': prev_data['all_pos'],
            'all_t0': prev_data['all_t0'],
            'all_scoring_energies': all_scoring_energies,
            'args': vars(args),
            'resampled_from': args.load_previous,
            'rescore_only': True,
        }
        if args.final_score_checkpoint:
            save_dict['final_score_checkpoint'] = args.final_score_checkpoint
        save_dict['scoring_time'] = args.scoring_time
        torch.save(save_dict, os.path.join(log_dir, 'dynamics_trajectory.pt'))
        print(f"Saved rescored energies to {log_dir}/dynamics_trajectory.pt")
    ...
    with torch.no_grad():
        for b in range(current_num_batches):
            ...
            for step_idx, t in enumerate(reverse_steps):
                ...
                # Temperature scaling: use temp_scaling_after_lo_time when t >= lo_time_threshold (default: same as current)
                effective_temp_scaling = (
                    args.temp_scaling_after_lo_time
                    if (args.lo_time_threshold != -1 and t >= args.lo_time_threshold and args.temp_scaling_after_lo_time != -1)
                    else current_temp_scaling
                )
                ...
                for i in range(current_iters + 1):
                    ...
                    input_feats = {
                        ...
                        'selfcond_coords' : prev0 if self_condition else torch.zeros_like(prev0),
                        'atom_mask' : atom_mask.unsqueeze(0).expand([bsize,-1,-1]).cuda()
                    }
                    if external_contacts is not None:
                        input_feats['external_contacts'] = external_contacts.unsqueeze(0).expand([bsize,-1]).cuda()


                    if metropolis and i < current_iters:

                        if score is None:
                            # Select model based on time threshold
                            current_model = get_dynamics_model(t)
                            
                            if use_aux_for_this_round:
                                out = current_model.compute_energy(input_feats)
                                score = out['r_update_aux']
                                energy = out['energy']
                            else:
                                out = current_model.compute_score(input_feats)
                        ...
                        next_mean = pos_t + diffuser.diffusion_coef(t)**2 * score * effective_temp_scaling / diffuser.config.coordinate_scaling * dt
                        ...
                        current_model = get_dynamics_model(t)
                        out_proposed = current_model.compute_score(input_feats_proposed)
                        ...
                        proposed_next_mean = pos_t_proposed + diffuser.diffusion_coef(t)**2 * score_proposed * effective_temp_scaling / diffuser.config.coordinate_scaling * dt

                        energy_ratio = torch.exp(-(out_proposed['energy'] - energy) * effective_temp_scaling)
                        ...
                        if i < current_iters:
                            pos_t = pos_t - (diffuser.drift_coef(pos_t, t) - diffuser.diffusion_coef(t)**2 * score * effective_temp_scaling /  diffuser.config.coordinate_scaling) * dt + diffuser.diffusion_coef(t) * np.sqrt(dt) * torch.randn_like(pos_t) / diffuser.config.coordinate_scaling
                        else:
                ...
                if args.scoring_time != -1 and t == reverse_steps[-1]:
                    ...
                    # Apply save stride to t0 positions being scored
                    t0_log_to_score = t0_log[::args.save_stride] if args.save_stride > 1 else t0_log
                    if args.save_stride > 1:
                        print(f"Scoring {len(t0_log_to_score)}/{len(t0_log)} frames (stride={args.save_stride})")
                    
                    for t0_step in t0_log_to_score:
                        # Extract CA coordinates from t0 prediction (all-atom structure)
                        t0_ca = t0_step[..., 1, :].reshape([bsize, -1, 3])
                        
                        # Create input features for energy scoring using t0 coordinates
                        scoring_input_feats = {
                            'r_noisy': t0_ca.cuda(),
                            'aatype': aatype.unsqueeze(0).expand([bsize,-1]).cuda(),
                            'mask': residue_mask.unsqueeze(0).expand([bsize,-1]).cuda(),
                        ...
                        if external_contacts is not None:
                            scoring_input_feats['external_contacts'] = external_contacts.unsqueeze(0).expand([bsize,-1]).cuda()
...
if args.scoring_time != -1:
    ...
    if external_contacts is not None:
        native_input_feats['external_contacts'] = external_contacts.unsqueeze(0).expand([bsize,-1]).cuda()
...
if args.resample_dynamics_config:
    save_dict['resample_dynamics_config'] = args.resample_dynamics_config
...
# Add low time model info if used
if args.lo_time_checkpoint and args.lo_time_threshold != -1:
    save_dict['lo_time_checkpoint'] = args.lo_time_checkpoint
    save_dict['lo_time_config'] = args.lo_time_config
    save_dict['lo_time_threshold'] = args.lo_time_threshold
    if args.temp_scaling_after_lo_time != -1:
        save_dict['temp_scaling_after_lo_time'] = args.temp_scaling_after_lo_time
