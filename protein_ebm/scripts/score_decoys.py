...
parser.add_argument('--decoy_dir', type=str, default='../../eval_data/decoys', help='Directory containing <name>.pt decoy files')
...
for d in all_decoys:
    print(d)
    decoys = torch.load(os.path.join(args.decoy_dir, f'{d}.pt'), weights_only=False)
    decoy_nrg = []
        ...
        for t in torch.linspace(args.t_min, args.t_max, args.n_samples):
            ...
            for i in range(args.ensemble_size if args.ensemble_size > 0 else 1):
                ...
                for i,batch in enumerate(train_loader):
                    ...
                    contacts = torch.ones(aatype.shape, dtype=torch.long) if getattr(config.model, 'num_contact_embeddings', 2) == 3 else torch.zeros(aatype.shape, dtype=torch.long)
