...
class AttentionPairBias(nn.Module):
    ...
    def forward(
        ...
        mask2d : torch.Tensor, optional
            The 2D pairwise mask tensor (B, N, N) to mask out specific attention weights
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            # Apply 2D mask if provided
            if mask2d is not None:
                attn = attn + (1 - mask2d[:, None, :, :].float()) * -self.inf
